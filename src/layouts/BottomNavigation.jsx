...
export const NavigationButton = ({title,Icon,route}) => {
   ...
   const onClasses = ({isActive}) => {
      ...
      return  `block  duration-150 bg-green-200 rounded-full ${activeStyle}`
   }
   return (
      <NavLink  to={route} className={onClasses} >
         <div className='flex flex-col  items-center justify-center text-xs md:text-sm  text-green-700  text-center  space-y-2 '>
...
export const BottomNavigation = ({className,height=60,...props}) => {
  ...
  return (
    <div 
      ref={ref}
      className={`fixed  z-50 bottom-0 left-0 w-full  bg-green-200 rounded-t-xl  flex justify-around items-center ${className}`} 
      style={{
         height,
         ...props.style
      }}
